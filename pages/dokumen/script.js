...
function renderSummary(){
    ...
    container.innerHTML = DOCUMENT_SUMMARY.map(item => `
...
function loadModuleInformation() {
    ...
    if (dashboardCount) {
        ...
        dashboardCount.textContent =
            `${DOCUMENT_CONFIG.totalDashboard} Dashboard`;

    }

    // Judul Browser
    document.title =
        `${DOCUMENT_CONFIG.title} | EMIS`;
...
function renderDocumentDashboards(filter="all") {

    const container =
        document.getElementById("document-dashboard");
    ...
    const dashboards =
        filter === "all"
        ? DOCUMENT_DASHBOARDS
        : DOCUMENT_DASHBOARDS.filter(item => item.type === filter);
...
function initTabs(){
    ...
    tabs.forEach(tab=>{
        ...
        tab.addEventListener("click",()=>{
            ...
            renderDocumentDashboards(tab.dataset.filter);
