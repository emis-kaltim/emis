...
function loadModuleInformation() {
    ...
    if (dashboardCount) {
        ...
        dashboardCount.textContent =
            `${EDUCATION_CONFIG.totalDashboard} Dashboard`;

    }

    // Judul Browser
    document.title =
        `${EDUCATION_CONFIG.title} | EMIS`;
...
function renderEducationDashboards(filter="all") {
    ...
    const dashboards =
        ...
        : EDUCATION_DASHBOARDS.filter(item => item.type === filter);
...
function initTabs(){
    ...
    tabs.forEach(tab=>{
        ...
        tab.addEventListener("click",()=>{
            ...
            renderEducationDashboards(tab.dataset.filter);
